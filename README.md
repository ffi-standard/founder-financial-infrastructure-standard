# Founder Financial Infrastructure Standard

The **first open framework** defining what financial infrastructure every growth‑focused company must maintain from first model to final exit.

- **Seven Books** covering financial architecture, performance modeling, capital structure, valuation, investor readiness, and strategic planning.
- **Three compliance levels** (Operational Foundation, Investor Readiness, Institutional Standard).
- **178 defined terms** with precise, citable definitions.
- Published under **Creative Commons Attribution 4.0 International** – free to use, cite, and distribute.

---

## Quick Links

- **Official website:** [ffistandard.org]((https://ffistandard.org/))
- **Download the complete Standard (PDF):** [Beta v0.5](https://ffistandard.org/assets/ffi-standard-complete-beta-v05.pdf)
- **Glossary:** [ffistandard.org/glossary/](https://ffistandard.org/glossary/)
- **Citation guide:** [ffistandard.org/cite/](https://ffistandard.org/cite/)

---

## About This Repository

This repository contains the full text of the Founder Financial Infrastructure Standard. It is maintained as a reference for contributors, translators, and implementers. The definitive version is always available at the official website above.

## License

[Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/)
